...
└── <source>/
    ...
    └── start/                # When using start generator
...
Tangrams can generate server functions for TanStack Start, wrapping your operations in `createServerFn` for server-side data fetching. For more details, see the [TanStack Start](/docs/tanstack-start) guide.

### Quick Setup

Add `"start"` to your `generates` array:
  ...
  sources: [
    {
      ...
      documents: "./src/graphql/**/*.graphql",
      generates: ["query", "start"], // Generate both
    },
  ],
})
```

To have query options use server functions:
...
Server functions are generated in a separate `start/` directory:
