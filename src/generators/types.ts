...
export function generateTypes(options: TypeGeneratorOptions): string {
  ...
  // Generate schema types (enums, input types)
  const schemaTypes = generateSchemaTypes(schema, scalars);
  if (schemaTypes) {
...
function generateSchemaTypes(
  ...
  for (const [name, type] of Object.entries(typeMap)) {
    ...
    if (isEnumType(type)) {
      lines.push(generateEnumType(type));
      lines.push("");
    }

    if (isInputObjectType(type)) {
      lines.push(generateInputType(type, scalars));
      lines.push("");
    }
  }
